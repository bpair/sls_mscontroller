...
const INTEGRATIONFLDS = {
    ...
    SENSORLSTTYP: 'sLstTyp' //Last Sensor Type Value
};
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            docClient.get(dbParams, function (err, data) {
                if (err) {
                    if (failsafe) {
                        logger.info("lookupIntegrationByKey DB Error (failsafe = true): ", err);
                        resolve({});
                    } else {
                        logger.error("lookupIntegrationByKey DB Error: ", err);
                        reject(err);
                    }
                } else {
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            docClient.update(dbParams, function (err, data) {
                if (err) {
                    if (failsafe) {
                        logger.info("Update Integration Attributes DB Error (failsafe = true): ", err);
                        resolve({});
                    } else {
                        logger.error("Update Integration Attributes DB Error: ", err);
                        reject(err);
                    }
                } else {
            ...
            logger.error("Update Integration Attributes Error: ", err);
            reject(err);
        }
    });
}
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            docClient.scan(dbParams, function (err, data) {
                if (err) {
                    logger.error("getGatewayNodeIds DB Error: ", err);
                    reject(err);
                } else {
            ...
            logger.error("getGatewayNodeIds Error: ", err);
            reject(err);
        }
    });
}
...
function getKeysByGatewayNodeId(gwId, nId, tbl_prefix) {
    return new Promise(function (resolve, reject) {
        try {
            let dbParams = {
                TableName: tbl_prefix + TABLENAME_ASSETINTEGRATION,
                FilterExpression: "#g = :gv and #n = :nv",
                ExpressionAttributeNames: {
                    '#g': INTEGRATIONFLDS.GWID,
                    '#n': INTEGRATIONFLDS.NODEID,
                    '#k': INTEGRATIONFLDS.KEY
                },
                ExpressionAttributeValues: {
                    ':gv': gwId,
                    ':nv': nId
                },
                ProjectionExpression: '#k ,' + INTEGRATIONFLDS.VRS
            };
            logger.info("Get Keys for a specific Gateway and Node - dbParams: %j", dbParams);
            docClient.scan(dbParams, function (err, data) {
                if (err) {
                    logger.error("getKeysByGatewayNodeId DB Error: ", err);
                    reject(err);
                } else {
            ...
            logger.error("getKeysByGatewayNodeId Error: ", err);
            reject(err);
        }
    });
}
