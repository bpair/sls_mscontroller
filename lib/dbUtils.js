...
function lookupIntegrationByKey(args, failsafe, tbl_prefix) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            logger.info("lookupAssetIntegration() dbParams: %j", dbParams);
            docClient.get(dbParams, function (err, data) {
                if (err) {
                    if (failsafe) {
                        logger.info("Asset Integration Lookup DB Error (failsafe = true): %j", err);
                        resolve({});
                    } else {
                        logger.info("Asset Integration Lookup DB Error: %j", err);
                        reject(err);
                    }
                } else {
                    logger.info("Asset Integration Lookup DB results: %j", data);
                    resolve(data);
                }
            });
        } catch (err) {
            logger.info("Asset Integration LookupDB Error: %j", err);
            reject(err);
        }
    });
}
...
function updateIntegrationAttributes(key, updateExp, condExp, attNames, attVals, tbl_prefix, failsafe) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            dbParams[INTEGRATIONFLDS.KEY] = key;
            ...
            logger.info("Update Integration Attributes Error: %j", err);
            reject(err);
        }
    });
}
...
function getGatewayNodeIds(gwId, tbl_prefix) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            logger.info("getGatewayNodeIds Error: %j", err);
            reject(err);
        }
    });
}
...
function getKeysByGatewayNodeId(gwId, nId, tbl_prefix) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            logger.info("getKeysByGatewayNodeId Error: %j", err);
            reject(err);
        }
    });
}
