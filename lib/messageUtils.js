...
var esEndpoint = null;
if (esDomain) {
    esEndpoint = new AWS.Endpoint(esDomain.endpoint);
}
        ...
        .then(function (data) {
            if (hasIntegrationRecord) {
                ...
                if (!_.has(state, dbUtils.INTEGRATIONFLDS.SENSORTYPEVAL) || event[MSGATT.VAL] !== state[dbUtils.INTEGRATIONFLDS.SENSORTYPEVAL]) {
                    ...
                    return dbUtils.updateIntegrationAttributes(event[MSGATT.KEY], updateExp, conditionExp, attNames, attValues, TBLPRE, false);
                }
                return data;
            }
        });
    ...
    return dbUtils.lookupIntegrationByKey(event, false, TBLPRE)
        ...
        .then(function (data) {
            if (!hasIntegrationRecord) {
                ...
                item[dbUtils.INTEGRATIONFLDS.SENSORLSTTYP] = event[MSGATT.TYP];
                return dbUtils.putAssetIntegration(item, TBLPRE);
            }
        })
        .then(function (data) {
            if (hasIntegrationRecord) {
                //update specific attributes
                let updateExp = 'set #st  = :stv, #stm = :stmv, #stmi = :stmiv, #stp = :stpv, #vrs = :vrsv2';
                let conditionExp = '#vrs  = :vrsv';
                let attNames = {
                    '#st': dbUtils.INTEGRATIONFLDS.SENSORLSTVAL,
                    '#stm': dbUtils.INTEGRATIONFLDS.SENSORLSTTEL,
                    '#stmi': dbUtils.INTEGRATIONFLDS.SENSORLSTTELISO,
                    '#stp': dbUtils.INTEGRATIONFLDS.SENSORLSTTYP,
                    '#vrs': dbUtils.INTEGRATIONFLDS.VRS
                };
                let attValues = {
                    ':stv': event[MSGATT.VAL],
                    ':stmv': event[MSGATT.TM],
                    ':stmiv': event[MSGATT.TMISO],
                    ':stpv': event[MSGATT.TYP],
                    ':vrsv': state[dbUtils.INTEGRATIONFLDS.VRS],
                    ':vrsv2': state[dbUtils.INTEGRATIONFLDS.VRS] + 1
                };
                return dbUtils.updateIntegrationAttributes(event[MSGATT.KEY], updateExp, conditionExp, attNames, attValues, TBLPRE, false);
            }
            return data;
        ...
        .then(function (data) {
            logger.debug('END:handleSetEvent %j', event);
            return data;
        });
}
...
function handleBatteryLevelEvent(event) {
    logger.debug("START: handleBatteryLevelEvent %j", event);
...
function handleConfigEvent(event) {
    logger.debug("START: handleConfigEvent %j", event);
...
function handleTimeRequestEvent(event) {
    logger.debug("START: handleTimeRequestEvent %j", event);
    let dt = new Date();
...
function handleNodeIdRequestEvent(event) {
    logger.debug("START: handleNodeIdRequestEvent %j", event);
...
function handleSketchNameEvent(event) {
    logger.debug("START: handleSketchNameEvent %j", event);
    //get keys
...
function handleSketchVersionEvent(event) {
    logger.debug("START: handleSketchVersionEvent %j", event);
    //get keys
    return dbUtils.getKeysByGatewayNodeId(event[MSGATT.GWID], event[MSGATT.NODEID], TBLPRE)
        .then(function (data) {
            if (data && data.hasOwnProperty('Items') && !_.isEmpty(data.Items)) {
...
function postHTTPMsg(topicName, msg, qos) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            logger.debug('postHTTPMsg: %j', params);
            if (iotdata) {
                iotdata.publish(params, function (err, data) {
                    if (err) {
                        logger.error('Error occurred publishing %s', err);
                        reject(err);
                    } else {
                        logger.debug('Publish success');
                        //                        logger.debug(data); // successful response
                        resolve("Message forwarded successfully");
                    }
                });
            } else {
            ...
            logger.error("IOT Error Posting HTTP Message: ", err);
            reject(err);
        }
    });
}
...
function postToES(doc, context) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            var indexName = [
                esDomain.index + timestamp.getUTCFullYear(), // year
            ...
            send.handleRequest(req, null, function (httpResp) {
                ...
                httpResp.on('end', function (chunk) {
                    logger.debug('postToES: Response: ' + respBody);
                    resolve('Success');
                });
            }, function (err) {
